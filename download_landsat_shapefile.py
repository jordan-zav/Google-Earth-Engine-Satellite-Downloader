...
PRODUCT_NAME = "landsat_oli_sr_indices"
EXPECTED_BANDS = 11
...
def build_image(region: ee.Geometry, start: str, end: str) -> ee.Image:
    ...
    blue = mosaic.select("SR_B2")
    green = mosaic.select("SR_B3")
    red = mosaic.select("SR_B4")
    nir = mosaic.select("SR_B5")
    swir1 = mosaic.select("SR_B6")
    swir2 = mosaic.select("SR_B7")

    indices = [
        red.divide(blue).rename("iron_oxide_red_blue"),
        swir1.divide(swir2).rename("clay_hydroxyl_swir1_swir2"),
        swir1.divide(nir).rename("ferrous_silicate_swir1_nir"),
        nir.subtract(red).divide(nir.add(red)).rename("ndvi"),
        green.subtract(nir).divide(green.add(nir)).rename("ndwi"),
    ]
    return mosaic.select(["SR_B2", "SR_B3", "SR_B4", "SR_B5", "SR_B6", "SR_B7"]).addBands(indices)
...
def run(args) -> None:
    ...
    if retry_failed or len(final_tiles) != len(tiles) or summary["invalid_outputs"]:
        raise RuntimeError(f"Finished with unresolved Landsat problems. See {log_dir / 'summary.json'}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Download normalized Landsat tiles for any shapefile.")
